...
\begin{tikzpicture}[node distance = 1.8cm, auto]
	\node [block-w] (cliente) {Client};
	\node [block-w, below of=cliente] (app) {App};
	\node [circle, below of=app] (qr) {QR Code};
	\node [block-w, below left of=qr] (inv) {Creator};
	\node [block-w, below right of=qr] (vend) {Seller};
	\node [block-y, below of=vend] (gpg1) {$ GPG_{1} $};
	\node [block-y, below of=inv] (gpg2) {$ GPG_{2} $};
	\node [block-y, below of=gpg1] (dir1) {Address and percentage fee};
	\node [block-y, below of=gpg2] (dir2) {Address and percentage fee};
	\node [circle, below of=dir1] (comp) {Verification};
	\node [if, below of=comp] (if) {IF};
